...
import {GraduationCap,ClipboardCheck,Users,ArrowLeft,Search,CheckCircle2,LogIn,LogOut,LockKeyhole,WifiOff,RefreshCcw,ShieldCheck} from 'lucide-react';
...
type KSession={id:string;group_id:string;name:string;planned_start:string;session_id:string|null;session_status:'not_started'|'active'|'closed';students:KStudent[];};
...
export default function Terminal(){
 ...
 !group?<><div className="terminal-title"><span className="step-dot">1</span><div><h1>Guruhingizni tanlang</h1><p>Davomat uchun guruh ustiga bosing</p></div></div>{loading&&!snapshot?<div className="notice">Tekshirilmoqda...</div>:error&&!snapshot?<div className="terminal-empty"><WifiOff size={46}/><h2>Server bilan aloqa yo‘q</h2><p>Internetni tekshiring. Xatolik bartaraf etilgach, qayta urinib ko‘ring.</p><button className="btn btn-outline" onClick={()=>reload(token)}><RefreshCcw size={17}/> Qayta urinish</button></div>:snapshot?.sessions.length?<div className="terminal-groups">{snapshot.sessions.map(s=><button key={s.id} className="terminal-group-card" disabled={s.session_status==='closed'} onClick={()=>setGroup(s.id)}><span className="group-symbol"><Users size={28}/></span><strong>{s.name}</strong><small>{s.session_status==='closed'?'Bugungi dars yakunlangan':s.students.length+' nafar o‘quvchi'}</small></button>)}</div>:<div className="terminal-empty"><GraduationCap size={46}/><h2>Hozircha faol guruh yo‘q</h2><p>Terminal muvaffaqiyatli ulangan. Administrator guruh qo‘shishi bilanoq bu yerda paydo bo‘ladi. Dars boshlanishini kutish shart emas.</p><button className="btn btn-outline" onClick={()=>reload(token)}><RefreshCcw size={17}/> Yangilash</button></div>}</>:
