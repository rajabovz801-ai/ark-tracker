...
export async function POST(req:Request){
 try{
  ...
  if(body.action==='snapshot'){
   const data=await rest('rpc/sa_kiosk_snapshot_v2',SUPABASE_PUBLISHABLE_KEY,{method:'POST',body:JSON.stringify({p_token:body.token})});
   return Response.json(data,{headers:{'Cache-Control':'no-store'}});
  }
  if(body.action==='in'||body.action==='out'){
   if(!isUuid(body.groupId)||!isUuid(body.studentId))throw new ApiError('O‘quvchi yoki guruh tanlanmagan');
   const data=await rest('rpc/sa_kiosk_event_v2',SUPABASE_PUBLISHABLE_KEY,{method:'POST',body:JSON.stringify({p_token:body.token,p_group_id:body.groupId,p_student_id:body.studentId,p_action:body.action})});
   return Response.json(data,{headers:{'Cache-Control':'no-store'}});
  }
