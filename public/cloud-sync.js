(() => {
  ...
  function normalizeWeeklyPointsState(data, at = new Date()) {
    ...
    if (data.weeklyPointsCycle === cycle) {
      return { data, changed: false };
    }
  ...
  async function fetchRemote() {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/ark_tracker_public_get_state`, {
      method: 'POST',
      headers: headers(),
      body: '{}',
      cache: 'no-store',
    ...
    if (!response.ok) {
      throw new Error(data?.message || `Cloud read failed: ${response.status}`);
    }
  ...
  async function pushRaw(raw) {
    ...
    let data;
    try {
      data = JSON.parse(normalizeRawState(raw));
    } catch (_) {
      return false;
    }

    const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/ark_tracker_public_save_state`, {
      method: 'POST',
      headers: headers(),
    ...
    if (!response.ok) {
      throw new Error(result?.message || `Cloud save failed: ${response.status}`);
    }
  ...
  function notifyError(error) {
    ...
    window.dispatchEvent(new CustomEvent('ark-cloud-error', {
      detail: { message: error?.message || 'Cloud sync unavailable.' },
    }));
  }
  ...
  async function initCloud() {
    ...
    try {
      ...
      cloudReady = false;
      console.warn('[ARK Tracker] Public cloud sync unavailable; local data was not deleted.', error);
      notifyError(error);
    }
  }
