...
export async function loadAdmin(req:Request){
 ...
 const [groups,students,memberships,sessions,events,devices,deletedRecords,preAttendance]=await Promise.all([
  ...
  rest('sa_pre_attendance?select=*&lesson_date=gte.'+from+'&lesson_date=lte.'+to+'&order=checked_in.desc&limit=5000',token)
 ]);
 ...
 return {groups,students,memberships,sessions,attendance,preAttendance,events,devices,deletedRecords,admin};
}
